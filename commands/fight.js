...
module.exports = {
    run: async (message) => {
        resultat = nombre[Math.floor(Math.random() * nombre.length)]
        ...
        if (resultat === "2") return message.channel.send(new Discord.MessageEmbed()
            ...
            .setTimestamp())
        else if (resultat === "4") message.channel.send(new Discord.MessageEmbed()
            .setTitle(`${message.author.username} se bats contre quelqu\'un :`)
            .setDescription(`<@${message.author.id}> mets par terre ${member} et lui explose le crâne, ce qui le tue.\n\n<@${message.author.id}> a gagné !`)
            .setTimestamp())
        else if (resultat === "1") message.channel.send(new Discord.MessageEmbed()
            .setTitle(`${message.author.username} se bats contre quelqu\'un :`)
            .setDescription(`${member} casse les deux bras à <@${message.author.id}>, ce qui le mets hors-combat.\n\n${member} a gagné !`)
            .setTimestamp())
        else if (resultat === "3") message.channel.send(new Discord.MessageEmbed()
            .setTitle(`${message.author.username} se bats contre quelqu\'un :`)
